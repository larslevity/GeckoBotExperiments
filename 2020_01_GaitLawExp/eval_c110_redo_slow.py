...
len_leg, len_tor = calibration.get_len(versions[0])
...
# %% Plot track for first impression
    ...
    plt.plot(t, eps, ls[version], color=color_eps[version])
...
# %% Bending angle plot
    ...
    for i in range(5):
        y = pf.downsample(alp[i], proportion=prop)
...
#        plt.plot(t, phi_1[i], color=get_feet_color()[i])
    ...
    plt.plot(x, y, ls[version], color='green')
    plt.fill_between(x, y+sig, y-sig,
                     facecolor=color_eps[version], alpha=0.5)
