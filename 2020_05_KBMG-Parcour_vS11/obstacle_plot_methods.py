# -*- coding: utf-8 -*-
"""
Created on Thu Jan 30 14:48:15 2020
...
def plot_track(db, POSE_IDX, save_as_tikz=False, show_cycles=1, tags=[1],
               **kwargs):
    prop = calc_prop(db)
    col = get_marker_color()

    plt.figure('Track')
    for exp_idx, dset in enumerate(db):
#        plt.figure('Track'+mode+str(exp_idx))
        for idx in tags:  # pos of tags
            x = dset['x{}'.format(idx)]
            y = dset['y{}'.format(idx)]
            ...
            if idx == 8:
                try:
                    plt.plot(x_, y_, 'o', color=col[idx])
                except KeyError:
                    plt.plot(x_, y_, 'o')
            else:
                try:
                    plt.plot(x_, y_, '-', color=col[idx], **kwargs)
                except KeyError:
